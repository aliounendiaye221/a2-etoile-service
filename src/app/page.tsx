      ...
      <SectionShell className="relative flex min-h-[88vh] items-center overflow-hidden bg-white sm:min-h-[95vh]">
        ...
        <Container className="relative z-10">
          <div className="grid gap-10 lg:grid-cols-2 lg:items-center lg:gap-12">
            <div className="animate-fade-up">
                 <div className="mb-8 flex flex-wrap items-center gap-3 sm:gap-4">
                   <div className="inline-flex max-w-full items-center gap-2 rounded-full border border-ocean-100 bg-ocean-50/50 px-4 py-2 text-[10px] font-black uppercase tracking-[0.12em] text-ocean-900 shadow-sm backdrop-blur-sm sm:gap-3 sm:px-5 sm:py-2.5 sm:tracking-[0.4em]">
                     Dakar & Rufisque : L&apos;Excellence Mobile
                   </div>
                   <div className="inline-flex max-w-full items-center gap-2 rounded-full bg-gold-500 px-4 py-2 text-[10px] font-black uppercase tracking-[0.12em] text-white shadow-lg animate-pulse sm:tracking-[0.2em]">
                 ...
                 {/* VISIBLE LOGO SLOGAN */}
                  <p className="mb-6 inline-block max-w-full rounded-r-lg border-l-4 border-gold-500 bg-gold-50/30 py-2 pl-4 text-xs font-black uppercase tracking-[0.18em] text-gold-600 sm:tracking-[0.5em]">
                    {company.slogan}
                 </p>
                 
                 <h1 className="font-display text-4xl leading-[0.95] tracking-tighter text-ocean-950 sm:text-6xl sm:leading-[0.9] lg:text-9xl">
                   L&apos;Art de la <br/>
                 ...
                 <div className="mt-10 flex flex-wrap items-center gap-4 sm:mt-12 sm:gap-6">
                   <PremiumLink href="/devis" variant="primary" className="btn-shimmer h-14 w-full justify-center rounded-2xl px-8 text-base shadow-glass hover:shadow-glass-hover sm:h-16 sm:w-auto sm:px-12 sm:text-xl">
          ...
          <div className="relative aspect-[4/5] w-full overflow-hidden rounded-[2.5rem] border-8 border-white shadow-2xl glass-card sm:rounded-[4rem] sm:border-[16px]">
                  <Image
                    src="/hero-villa.webp"
                    alt="Nettoyage de Villas d'Exception à Dakar - A2 Étoile Service"
                    fill
      ...
      <section className="py-16 bg-white border-y border-ocean-50">
        <Container>
          <div className="grid grid-cols-2 gap-6 sm:gap-8 lg:grid-cols-4 lg:gap-12">
            {stats.map((stat) => (
              <div key={stat.label} className="group min-w-0">
                ...
                <p className="break-words text-[10px] font-black uppercase tracking-[0.15em] text-ocean-600 sm:tracking-[0.3em]">
                  {stat.label}
                </p>
              </div>
            ))}
      ...
      <SectionShell className="bg-ocean-50/30">
        <Container>
          ...
          <div className="mt-14 grid gap-8 sm:mt-20 sm:gap-10 md:grid-cols-2 lg:grid-cols-3">
             ...
             <ServiceCard service={services[0]} className="lg:col-span-2 lg:h-[600px]" highlight variant="overlay" />
             
             {/* NETTOYAGE VILLA */}
             <ServiceCard service={services[1]} />
             
             {/* REST OF SERVICES */}
             {services.slice(2).map((service, index, array) => {
      ...
      <SectionShell>
        <Container>
          <div className="grid items-center gap-12 lg:grid-cols-2 lg:gap-20">
               ...
               <div className="mt-8 space-y-6 sm:mt-10 sm:space-y-8">
                 {[
                   ...
                   <div key={item.title} className="flex gap-5 group">
                    <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-gold-50 text-gold-600 shadow-sm transition-all duration-300 group-hover:bg-gold-500 group-hover:text-white sm:h-14 sm:w-14 sm:rounded-2xl">
                      <item.icon className="h-6 w-6" />
                     </div>
                     <div>
                       <h3 className="text-base font-bold text-ocean-950 sm:text-xl">{item.title}</h3>
                       <p className="mt-1.5 text-sm text-clean-800 font-medium leading-relaxed sm:mt-2">{item.desc}</p>
                     </div>
                   </div>
                 ))}
               </div>
            ...
            <div className="relative group order-1 overflow-hidden rounded-[2.5rem] lg:order-2 sm:rounded-[4rem]">
               <Image 
                  src="/b2b-corporate.webp" 
                  alt="Société de Nettoyage de Bureaux et B2B à Dakar - A2 Étoile Service"
                  width={800}
                  height={1000}
                  className="w-full rounded-[2.5rem] object-cover transition-transform duration-1000 group-hover:scale-105 sm:rounded-[4rem]"
                  sizes="(max-width: 1024px) 100vw, 50vw"
               />
               <div className="absolute inset-0 bg-ocean-950/20" />
               <div className="absolute bottom-4 left-4 right-4 flex flex-col items-start gap-3 rounded-[1.5rem] border border-white bg-white/80 p-4 shadow-2xl backdrop-blur-xl sm:bottom-12 sm:left-12 sm:right-12 sm:flex-row sm:items-center sm:justify-between sm:rounded-[3rem] sm:p-8">
                  <div>
                    <p className="font-display text-lg text-ocean-950 sm:text-2xl">Expertise B2B</p>
                    <p className="text-xs text-ocean-600 font-bold uppercase tracking-widest mt-1 sm:text-sm">Secteur Tertiaire & Industriel</p>
                  </div>
                  <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-ocean-900 text-white sm:h-14 sm:w-14">
         ...
         <div className="relative overflow-hidden rounded-[2.5rem] bg-gradient-to-br from-ocean-900 to-ocean-950 p-8 text-center shadow-2xl sm:rounded-[5rem] sm:p-16 lg:p-24">
           ...
           <Quote className="absolute left-6 top-6 hidden h-20 w-20 text-white/5 sm:block lg:left-10 lg:top-10 lg:h-24 lg:w-24" />
             ...
             <div className="mt-10 flex flex-wrap justify-center gap-4 sm:mt-16 sm:gap-8">
               <PremiumLink href="/devis" variant="primary" className="btn-shimmer h-14 w-full justify-center rounded-2xl px-8 text-base shadow-2xl sm:h-20 sm:w-auto sm:rounded-[2rem] sm:px-16 sm:text-2xl">
